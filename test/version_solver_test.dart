...
void unsolvable() {
  test('no version that matches constraint', () async {
    ...
    await expectResolves(error: equalsIgnoringWhitespace("""
          Because no versions of foo match ^1.0.0 and myapp depends on foo
  ...
  test('disjoint constraints', () async {
    ...
    await d.appDir({'foo': '1.0.0', 'bar': '1.0.0'}).create();
    await expectResolves(error: equalsIgnoringWhitespace("""
          Because all versions of foo depend on shared <=2.0.0 and all versions
            of bar depend on shared >3.0.0, foo is incompatible with bar.
          So, because myapp depends on both bar 1.0.0 and foo 1.0.0, version
            solving failed.
        """));
  });
  ...
  test('mismatched sources', () async {
    ...
    await expectResolves(error: equalsIgnoringWhitespace("""
          Because all versions of foo depend on shared 1.0.0 from hosted and all
            versions of bar depend on shared any from path, foo is incompatible
            with bar.
          So, because myapp depends on both bar 1.0.0 and foo 1.0.0, version
            solving failed.
        """));
  ...
  test('no valid solution', () async {
    ...
    await expectResolves(error: equalsIgnoringWhitespace("""
          Because b <2.0.0 depends on a 2.0.0 which depends on b 2.0.0, b <2.0.0
            is forbidden.
          Because b >=2.0.0 depends on a 1.0.0 which depends on b 1.0.0, b
            >=2.0.0 is forbidden.
          Thus, b is forbidden.
...
void backtracking() {
  ...
  test('failing backjump to conflicting source', () async {
    ...
    await d.appDir({'a': 'any', 'b': 'any', 'c': 'any'}).create();
    await expectResolves(error: equalsIgnoringWhitespace("""
          Because all versions of b depend on a any from path and myapp depends
            on a any from hosted, b is forbidden.
          So, because myapp depends on b any, version solving failed.
        """));
  });
